<x-app-layout>
    ...
    <div class="p-3 overflow-hidden bg-white mb-3 rounded-md shadow-md dark:bg-dark-eval-1">
        ...
        <form action="{{ route('kasir.peminjaman.store') }}" method="POST" class="px-3">
            @csrf
            <div class="grid gap-3 md:grid-cols-3">
                <div>
                    ...
                    <input type="text" id="kode_transaksi" name="kode_transaksi"
                        ...
                        value="{{ $kodeTransaksi ?? old('kode_transaksi', 'TRX-' . now()->format('YmdHis')) }}" readonly />
                    @error('kode_transaksi')
                        <p class="mt-[5px] text-xs text-red-700">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    <label for="anggota_id" class="block text-sm font-medium text-gray-900 dark:text-white">
                        {{ __('Nama Anggota :') }}
                    </label>
                ...
                <div>
                    <label for="buku_id" class="block text-sm font-medium text-gray-900 dark:text-white">
                        {{ __('Judul Buku :') }}
                    </label>
                        ...
                        <option>-- Pilih --</option>
                            @foreach ($buku as $item)
                                <option value="{{ $item->id }}" 
                                    {{ $item->jumlah_stok <= 0 ? 'disabled class=text-gray-400' : '' }}
                                    {{ (collect(old('buku_id'))->contains($item->id)) ? 'selected' : '' }}>
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3 text-center">{{ __('Tanggal Pinjam') }}</th>
                    ...
                    <tbody>
                        ...
                        <div id="modal-detail" tabindex="-1" aria-hidden="true"
                            ...
                            <div class="bg-white dark:bg-gray-700 rounded-lg w-full max-w-md p-5 relative shadow">
                                ...
                                <div class="mt-4 space-y-2">
                                    ...
                                    <p><strong>Tanggal Pinjam:</strong> <span id="detail-tanggal"></span></p>
                        ...
                        <div id="modal-edit" tabindex="-1" aria-hidden="true" class="hidden fixed inset-0 z-50 justify-center items-center bg-black/50">
                            <div class="bg-white dark:bg-gray-700 rounded-lg w-full max-w-md p-5 relative shadow">
                                ...
                                <!-- Modal Body -->
                                <form id="form-edit-peminjaman" method="POST" class="mt-4 space-y-3">
                                    @csrf
                                    @method('PUT')
                                    ...
                                    <div>
                                        <label for="edit-kode-transaksi" class="block text-sm font-medium text-gray-900 dark:text-white">Kode Transaksi:</label>
                                        <input type="text" id="kode_transaksi" name="kode_transaksi"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm/6 rounded-lg focus:ring-primary focus:border-primary block w-full py-1.5 pl-3 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary dark:focus:border-primary"
                                        value="{{ $kodeTransaksi ?? old('kode_transaksi', 'TRX-' . now()->format('YmdHis')) }}" readonly />
                                    </div>
    ...
    const editButtons = document.querySelectorAll("[data-modal-target='modal-edit']");
        ...
        editButtons.forEach(btn => {
            btn.addEventListener("click", () => {
                ...
                document.getElementById("edit-buku").value = buku;

                // Set form action dinamis
                const form = document.getElementById("form-edit-peminjaman");
                form.action = `/admin/peminjaman/${uuid}`; // pastikan route sesuai
            });
        });
    });
</script>