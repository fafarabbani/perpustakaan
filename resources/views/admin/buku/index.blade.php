    ...
    <div class="p-3 overflow-hidden bg-white mb-3 rounded-md shadow-md dark:bg-dark-eval-1">
        ...
        <form action="" method="POST" class="px-3">
            ...
            <div class="grid gap-3 md:grid-cols-4">
                {{--  <div>
                    <label for="kode_buku" class="block text-sm font-medium text-gray-900 dark:text-white">
                        {{ __('Kode Buku :') }}
                    </label>
                ...
                <div>
                    <label for="judul" class="block text-sm font-medium text-gray-900 dark:text-white">
                        {{ __('Judul :') }}
                    </label>
                ...
                <div>
                    <label for="penerbit" class="block text-sm font-medium text-gray-900 dark:text-white">
                        {{ __('Penerbit :') }}
                    </label>
                ...
                <div>
                    <label for="dimensi" class="block text-sm font-medium text-gray-900 dark:text-white">
                        {{ __('Dimensi :') }}
                    </label>
                    <input type="text" id="dimensi" name="dimensi"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm/6 rounded-lg focus:ring-primary focus:border-primary block w-full py-1.5 pl-3 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary dark:focus:border-primary"
                        value="{{ old('dimensi') }}" required />
                    @error('dimensi')
                        <p class="mt-[5px] text-xs text-red-700">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    <label for="jumlah_stok" class="block text-sm font-medium text-gray-900 dark:text-white">
                        {{ __('Jumlah Stok :') }}
                    </label>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            <th scope="col" class="px-4 py-3 text-center">{{ __('Kode Buku') }}</th>
                            <th scope="col" class="px-4 py-3 text-center">{{ __('Judul') }}</th>
                            <th scope="col" class="px-4 py-3 text-center">{{ __('Penerbit') }}</th>
                            <th scope="col" class="px-4 py-3 text-center">{{ __('Dimensi') }}</th>
                            <th scope="col" class="px-4 py-3 text-center">{{ __('Jumlah Stok') }}</th>
                    ...
                    <tbody>
                            ...
                            <tr class="border-b dark:border-gray-700">
                                ...
                                <td class="px-2 py-3 flex items-center justify-center gap-1">
                                    <x-button class="hover:underline" size="sm" variant="info"
                                        ...
                                        data-jumlah-stok="{{ $item->jumlah_stok }}">
                                        {{ __('Detail') }}
                                    </x-button>
                                    <x-button 
                                    ...
                                    <form action="{{ route('admin.buku.destroy', $item->uuid) }}"
                                        method="POST" onsubmit="return confirm('Apakah kamu yakin ingin menghapus buku ini?')">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <div id="modal-detail" tabindex="-1" aria-hidden="true"
                            ...
                            <div class="bg-white dark:bg-gray-700 rounded-lg w-full max-w-md p-5 relative shadow">
                                ...
                                <div class="mt-4 space-y-2">
                                    <p><strong>Kode Buku:</strong> <span id="detail-kode-buku"></span></p>
                                    <p><strong>Judul:</strong> <span id="detail-judul"></span></p>
                                    <p><strong>Penerbit:</strong> <span id="detail-penerbit"></span></p>
                                    <p><strong>Dimensi:</strong> <span id="detail-dimensi"></span></p>
                                    <p><strong>Jumlah Stok:</strong> <span id="detail-jumlah-stok"></span></p>
                                </div>
                        ...
                        <!-- Modal Edit Buku -->
                        <div id="modal-edit" tabindex="-1" aria-hidden="true" class="hidden fixed inset-0 z-50 justify-center items-center bg-black/50">
                            <div class="bg-white dark:bg-gray-700 rounded-lg w-full max-w-md p-5 relative shadow">
                                <!-- Modal Header -->
                                <div class="flex justify-between items-center border-b pb-3">
                                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Edit Buku</h3>
                                    <button type="button" data-modal-hide="modal-edit"
                                        class="text-gray-400 hover:bg-gray-200 hover:text-gray-900 rounded-lg p-2 dark:hover:bg-gray-600 dark:hover:text-white">✕</button>
                                </div>

                                <!-- Modal Body -->
                                <form id="form-edit-buku" method="POST" class="mt-4 space-y-3">
                                    @csrf
                                    @method('PUT')
                                    ...
                                    <div>
                                        <label for="edit-kode-buku" class="block text-sm font-medium text-gray-900 dark:text-white">Kode Buku:</label>
                                        <input type="text" name="kode_buku" id="edit-kode-buku" class="w-full px-3 py-1.5 border rounded-lg" readonly>
                                    </div>

                                    <div>
                                        <label for="edit-judul" class="block text-sm font-medium text-gray-900 dark:text-white">Judul:</label>
                                        <input type="text" name="judul" id="edit-judul" class="w-full px-3 py-1.5 border rounded-lg" required>
                                    </div>

                                    <div>
                                        <label for="edit-penerbit" class="block text-sm font-medium text-gray-900 dark:text-white">Penerbit:</label>
                                        <input type="text" name="penerbit" id="edit-penerbit" class="w-full px-3 py-1.5 border rounded-lg" required>
                                    </div>

                                    <div>
                                        <label for="edit-dimensi" class="block text-sm font-medium text-gray-900 dark:text-white">Dimensi:</label>
                                        <input type="text" name="dimensi" id="edit-dimensi" class="w-full px-3 py-1.5 border rounded-lg">
                                    </div>

                                    <div>
                                        <label for="edit-jumlah-stok" class="block text-sm font-medium text-gray-900 dark:text-white">Jumlah Stok:</label>
                                        <input type="number" name="jumlah_stok" id="edit-jumlah-stok" class="w-full px-3 py-1.5 border rounded-lg" required>
                                    </div>

                                    <!-- Modal Footer -->
                                    <div class="flex justify-end mt-5 border-t pt-3">
                                        <button type="button" data-modal-hide="modal-edit" class="px-4 py-2 rounded-lg bg-gray-500 text-white mr-2 hover:bg-gray-600">Batal</button>
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        buttons.forEach(btn => {
            btn.addEventListener("click", () => {
                ...
                const kode_buku = btn.getAttribute("data-kode-buku");
                const judul = btn.getAttribute("data-judul");
                const penerbit = btn.getAttribute("data-penerbit");
                const dimensi = btn.getAttribute("data-dimensi");
                const jumlah_stok = btn.getAttribute("data-jumlah-stok");

                // Isi modal
                document.getElementById("detail-kode-buku").textContent = kode_buku;
                document.getElementById("detail-judul").textContent = judul;
                document.getElementById("detail-penerbit").textContent = penerbit;
                document.getElementById("detail-dimensi").textContent = dimensi;
                document.getElementById("detail-jumlah-stok").textContent = jumlah_stok;
            });
        });
    });

    document.addEventListener("DOMContentLoaded", () => {
    const editButtons = document.querySelectorAll("[data-modal-target='modal-edit']");
