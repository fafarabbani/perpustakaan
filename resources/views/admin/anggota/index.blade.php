<x-app-layout>
    ...
    <div class="p-3 overflow-hidden bg-white mb-3 rounded-md shadow-md dark:bg-dark-eval-1">
        ...
        <form action="{{ route('admin.anggota.store') }}" method="POST" class="px-3">
            @csrf
            <div class="grid gap-3 md:grid-cols-3">
                <div>
                    <label for="nomor_anggota" class="block text-sm font-medium text-gray-900 dark:text-white">
                        {{ __('Nomor Anggota :') }}
                    </label>
                    <input type="text" id="nomor_anggota" name="nomor_anggota"
                        ...
                        value="{{ $nomorAnggota ?? old('nomor_anggota') }}" readonly />
                    @error('nomor_anggota')
                        <p class="mt-[5px] text-xs text-red-700">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    <label for="nama" class="block text-sm font-medium text-gray-900 dark:text-white">
                        {{ __('Nama :') }}
                    </label>
                ...
                <div>
                    <label for="tanggal_lahir" class="block text-sm font-medium text-gray-900 dark:text-white">
                        {{ __('Tanggal Lahir :') }}
                    </label>
                ...
                <div>
                    <label for="telepon" class="block text-sm font-medium text-gray-900 dark:text-white">
                        {{ __('Telepon :') }}
                    </label>
                ...
                <div>
                    <label for="alamat" class="block text-sm font-medium text-gray-900 dark:text-white">
                        {{ __('Alamat :') }}
                    </label>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            <th scope="col" class="px-4 py-3 text-center">{{ __('Nomor Anggota') }}</th>
                            <th scope="col" class="px-4 py-3 text-center">{{ __('Nama') }}</th>
                            <th scope="col" class="px-4 py-3 text-center">{{ __('Tanggal Lahir') }}</th>
                            <th scope="col" class="px-4 py-3 text-center">{{ __('Telepon') }}</th>
                            <th scope="col" class="px-4 py-3 text-center">{{ __('Alamat') }}</th>
                    ...
                    <tbody>
                            ...
                            <tr class="border-b dark:border-gray-700">
                                ...
                                <td class="px-2 py-3 flex items-center justify-center gap-1">
                                    <x-button class="hover:underline" size="sm" variant="info"
                                        ...
                                        data-nomor-anggota="{{ $item->nomor_anggota }}"
                                        data-nama="{{ $item->nama }}"
                                        data-tanggal-lahir="{{ $item->tanggal_lahir_formatted }}"
                                    ...
                                    <form action="{{ route('admin.anggota.destroy', $item->uuid) }}"
                                        method="POST" onsubmit="return confirm('Apakah kamu yakin ingin menghapus anggota ini?')">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <div id="modal-edit" tabindex="-1" aria-hidden="true" class="hidden fixed inset-0 z-50 justify-center items-center bg-black/50">
                            <div class="bg-white dark:bg-gray-700 rounded-lg w-full max-w-md p-5 relative shadow">
                                ...
                                <!-- Modal Body -->
                                <form id="form-edit-anggota" method="POST" class="mt-4 space-y-3">
                                    @csrf
                                    @method('PUT')
                                    ...
                                    <div>
                                        <label for="edit-nomor-anggota" class="block text-sm font-medium text-gray-900 dark:text-white">Nomor Anggota:</label>
                                        <input type="text" name="nomor_anggota" id="edit-nomor-anggota" class="w-full px-3 py-1.5 border rounded-lg" readonly>
                                    </div>

                                    <div>
                                        <label for="edit-nama" class="block text-sm font-medium text-gray-900 dark:text-white">Nama:</label>
                                        <input type="text" name="nama" id="edit-nama" class="w-full px-3 py-1.5 border rounded-lg" required>
                                    </div>

                                    <div>
                                        <label for="edit-tanggal-lahir" class="block text-sm font-medium text-gray-900 dark:text-white">Tanggal Lahir:</label>
                                        <input type="date" name="tanggal_lahir" id="edit-tanggal-lahir" class="w-full px-3 py-1.5 border rounded-lg" required>
                                    </div>

                                    <div>
                                        <label for="edit-telepon" class="block text-sm font-medium text-gray-900 dark:text-white">Telepon:</label>
                                        <input type="text" name="telepon" id="edit-telepon" class="w-full px-3 py-1.5 border rounded-lg">
                                    </div>

                                    <div>
                                        <label for="edit-alamat" class="block text-sm font-medium text-gray-900 dark:text-white">Alamat:</label>
                                        <input type="text" name="alamat" id="edit-alamat" class="w-full px-3 py-1.5 border rounded-lg" required>
                                    </div>
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        buttons.forEach(btn => {
            btn.addEventListener("click", () => {
                ...
                const tanggal_lahir = btn.getAttribute("data-tanggal-lahir");
                ...
                document.getElementById("detail-tanggal-lahir").textContent = tanggal_lahir;
    ...
    const editButtons = document.querySelectorAll("[data-modal-target='modal-edit']");
        ...
        editButtons.forEach(btn => {
            btn.addEventListener("click", () => {
                ...
                document.getElementById("edit-nomor-anggota").value = nomor_anggota;
                document.getElementById("edit-nama").value = nama;
                document.getElementById("edit-telepon").value = telepon;
                document.getElementById("edit-tanggal-lahir").value = tanggal_lahir;
                document.getElementById("edit-alamat").value = alamat;

                // Set form action dinamis
                const form = document.getElementById("form-edit-anggota");
                form.action = `/admin/anggota/${uuid}`; // pastikan route sesuai
            });
        });
    });
</script>
